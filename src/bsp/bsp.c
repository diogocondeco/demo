...
static bsp_button_event_cfg_t m_events_list[BUTTONS_NUMBER] = { {BSP_EVENT_NOTHING, BSP_EVENT_DEFAULT}};

static uint8_t long_push_type = 0;
extern treqio_system_status_t treqioStatus;

/*
 * Temp fix no led support
 */
extern nrf_drv_twi_t twi;

app_timer_id_t m_button_timer_id;
//static void bsp_button_event_handler(uint8_t pin_no, uint8_t button_action);
static void treq_button_event_handler(uint8_t pin_no, uint8_t button_action);
#endif // BSP_SIMPLE
...
{
#ifdef BSP_BUTTON_0
	{	BSP_BUTTON_0, false, BUTTON_PULL, treq_button_event_handler},
#endif // BUTTON_0
...
//
			if (long_push_type == 1) {
				if (treqioStatus.deviceStatus != TREQIO_DEVICE_OFF) {
					nrf_gpio_pin_clear(LED_RED);
					treqioStatus.deviceStatus = TREQIO_DEVICE_OFF;
					app_sched_event_put(NULL, 0, treqio_device_init_shutdown_event, SCHEDULER_PRIORITY_LOW);
//					nrf_gpio_pin_clear (SYSTEM_POWER_CTRL);
				} else {
					treqioStatus.deviceStatus = TREQIO_DEVICE_BOOTING;
					nrf_gpio_pin_set (LED_GREEN);
				}
				long_push_type = 0;
			} else if (long_push_type == 2) {
				treqioStatus.deviceStatus = TREQIO_DEVICE_OFF;
				NVIC_SystemReset();
...
////							nrf_gpio_pin_set (SYSTEM_POWER_CTRL);
//						}
//					} else if(nrf_gpio_pin_read(DEVICE_BUTTON) == 0) {
					nrf_gpio_pin_set (SYSTEM_POWER_CTRL);
//					}
				} else {
					nrf_gpio_pin_clear(LED_RED);
//					long_push_type = 2;
					err_code = app_timer_start(m_button_timer_id, BSP_MS_TO_TICK(BSP_LONG_PUSH_DFU_TIMEOUT_MS), NULL);
					APP_ERROR_CHECK(err_code);
				}
			} else if(long_push_type == 1) {
				long_push_type = 2;
				nrf_gpio_pin_clear(LED_GREEN);
...
static void button_timer_handler(void * p_context) {
	treq_button_event_handler(*(uint8_t *)p_context, BSP_BUTTON_ACTION_LONG_PUSH);
}
...
#ifdef DEBUG_INIT_REGS
	SEGGER_RTT_printf(0, "Resetreas: 0x%.8X\r\n", NRF_POWER->RESETREAS);
	SEGGER_RTT_printf(0, "GPREGRET: 0x%.8X\r\n", NRF_POWER->GPREGRET);
#endif
...
//	if ((nrf_gpio_pin_read(DEVICE_BUTTON) == 0) || NRF_POWER->GPREGRET == 0xA0) {
	if (NRF_POWER->GPREGRET == 0xFF) {
		NRF_POWER->GPREGRET = 0;
//		nrf_gpio_cfg_input(SYSTEM_POWER_CTRL, NRF_GPIO_PIN_PULLDOWN);
		nrf_gpio_pin_clear (SYSTEM_POWER_CTRL);
		NVIC_SystemReset();
	} else if ((nrf_gpio_pin_read(DEVICE_BUTTON) == 0)) {
		if ((NRF_POWER->RESETREAS & POWER_RESETREAS_SREQ_Msk) != 0) {
			NRF_POWER->RESETREAS = 0xFFFFFFFF;
			NVIC_SystemReset();
		}
//		if (nrf_gpio_pin_nrf_gpio_pin_set (SYSTEM_POWER_CTRL);
//		if (nrf_gpio_pin_read(DEVICE_BUTTON) != 0) {
//			nrf_gpio_pin_set (SYSTEM_POWER_CTRL);
//
//		}
...
//		if ((nrf_gpio_pin_read(DEVICE_BUTTON) == 0)) {
		...
		err_code = app_timer_start(m_button_timer_id, BSP_MS_TO_TICK(BSP_LONG_PUSH_TIMEOUT_MS), NULL);
		APP_ERROR_CHECK(err_code);
...
//		}
		treqioStatus.deviceStatus = TREQIO_DEVICE_OFF;
//	} else if (NRF_POWER->GPREGRET == 0xA0) {
//		nrf_gpio_pin_set (SYSTEM_POWER_CTRL);
//		NRF_POWER->GPREGRET = 0;
//		NRF_POWER->RESETREAS = 0xFFFFFFFF;
//		nrf_gpio_pin_clear(SYSTEM_POWER_CTRL);
	} else if ((NRF_POWER->GPREGRET == 0xA0) && (nrf_gpio_pin_read(DEVICE_BUTTON) != 0)) {
		nrf_gpio_pin_set (SYSTEM_POWER_CTRL);
		nrf_gpio_pin_clear (LED_GREEN);
//		NRF_POWER->GPREGRET = 0;
		NRF_POWER->RESETREAS = 0xFFFFFFFF;
...
//		err_code = app_timer_start(m_button_timer_id, BSP_MS_TO_TICK(BSP_LONG_PUSH_TIMEOUT_MS), NULL);
//		treqioStatus.deviceStatus = TREQIO_DEVICE_BOOTING;
//	} else if ((NRF_POWER->RESETREAS ^ POWER_RESETREAS_DOG_Msk) != 0) {
//		nrf_gpio_pin_set (SYSTEM_POWER_CTRL);
////		NRF_POWER->RESETREAS |= POWER_RESETREAS_DOG_Msk;
//		long_push_type = 1;
//		treqioStatus.deviceStatus = TREQIO_DEVICE_BOOTING;
//	} else if ((NRF_POWER->RESETREAS & POWER_RESETREAS_SREQ_Msk) != 0) {
//		nrf_gpio_pin_set (SYSTEM_POWER_CTRL);
